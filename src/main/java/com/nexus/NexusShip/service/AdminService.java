...
public class AdminService {
    ...
    public AdminResponse registerAdmin(AdminRegistrationRequest request) {
        ...
        if (existingUser.isPresent()) {
            ...
            if (driver.isPresent()) {
                throw new AlreadyExistsException("This user is an active driver. He must be deleted from drivers to become an Admin");
            }
            ...
            if (existingAdminId.isPresent()) {
                ...
                Admin admin = adminRepository.findByIdEverywhere(existingAdminId.get())
                        .orElseThrow(()-> new NotFoundException("Error during restore the admin"));
                if(admin.isDeleted()){
                    ...
                    throw new AlreadyExistsException("This admin is already exist and active.");
                }
        ...
        if (userRepository.findByNationalId(request.nationalId()).isPresent()) {
            throw new AlreadyExistsException("National ID is already in use.");
        }
    ...
    public AdminResponse upgradeUserToAdmin(Long userId) {
        ...
        Admin admin = adminRepository.findByIdEverywhere(userId)
                .orElseThrow(() -> new NotFoundException("Error during upgrading to admin"));
        admin.setHireDate(LocalDateTime.now());
        admin.setSalary(INITIAL_SALARY);
        return adminMapper.toResponse(adminRepository.save(admin));
    ...
    public AdminResponse findAdminById(Long id) {
        return adminRepository.findById(id).
                ...
                .orElseThrow(() -> new NotFoundException("There is no admin with id " + id)
                );
    }

    @Transactional
    public void deleteAdmin(Long id) {
        Admin admin = adminRepository.findById(id).orElseThrow(() -> new NotFoundException("There is no admin with id " + id));
